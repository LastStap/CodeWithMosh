...
|---|---|---|---|
| class | Шаблон для створення об'єктів | Клас Car | Думати, що клас і об'єкт — це одне й те саме |
| object / instance |Конкретний екземпляр класу  |Створити автомобіль із класу Car  | Викликати методи на null-посиланні |
| field |Змінна, що належить об'єкту  | color, speed у класі Car | Плутати поле з локальною змінною |
| method |Дія, яку може виконувати об'єкт  | startEngine() | Забути викликати метод через () |
| constructor |Спеціальний метод для створення та ініціалізації об'єкта | Створити Car("BMW") | Думати, що конструктор можна викликати як звичайний метод |
| parameter |Змінна, що передається в метод або конструктор  |Метод приймає speed  |Плутати параметр з аргументом  |
| local variable | Змінна, створена всередині методу | int sum = 0; | Використати до ініціалізації |
| reference |Посилання на об'єкт у Heap  |Car car = new Car(); (car — reference)  | Думати, що reference містить сам об'єкт|
| state | Поточні значення полів об'єкта | color = "Red", speed = 120 | Плутати стан з поведінкою |
| behavior |Дії, які виконує об'єкт  |drive(), stop()  | Зберігати дані в методах замість полів |
| `new` | Створює новий об'єкт у Heap |new Car()  | Забути new перед створенням об'єкта |
| stack frame |Область Stack для одного виклику методу  | Виклик main() створює свій stack frame | Думати, що об'єкти зберігаються у Stack |
| heap |Область пам'яті, де зберігаються об'єкти  | Об'єкт new Car() | Думати, що примітиви теж завжди лежать у Heap |
| garbage collection |Автоматичне звільнення пам'яті від непотрібних об'єктів  |Об'єкт без жодного reference буде видалений GC  |  Очікувати, що GC спрацює одразу після obj = null|

------------------------

1. Чому class називають шаблоном, але class не є готовим object?
Бо class лише описує, яким має бути об'єкт. Об'єкт з'являється тільки після new.
Приклад: креслення будинку — це не сам будинок.

2. Чи можуть два objects одного class мати різний state?
Так. Вони мають однакову структуру, але різні значення полів.
Приклад: два автомобілі класу Car: один червоний, інший синій.

3. Чи може одна reference в різний час посилатися на різні objects?
Так. Reference можна переприсвоїти іншому об'єкту.
Приклад: спочатку car посилається на BMW, потім — на Audi.

4. Чи може object існувати без доступної reference у програмі?
Так. Якщо на нього більше немає жодного посилання, він ще деякий час існує в Heap, поки його не видалить Garbage Collector.
Приклад: створили об'єкт і втратили всі посилання на нього.

5. Чим null відрізняється від object із порожніми fields?
null означає, що об'єкта немає взагалі. Об'єкт із порожніми полями існує, просто його поля мають порожні або початкові значення.
Приклад: порожній будинок існує, а null — це взагалі відсутність будинку.

6. Чому порівнювати references — не завжди те саме, що порівнювати вміст?
Бо reference порівнює, чи це один і той самий об'єкт, а не чи однакові його дані.
Приклад: два різні об'єкти Person можуть мати однакові ім'я та вік, але це все одно різні об'єкти в пам'яті.
